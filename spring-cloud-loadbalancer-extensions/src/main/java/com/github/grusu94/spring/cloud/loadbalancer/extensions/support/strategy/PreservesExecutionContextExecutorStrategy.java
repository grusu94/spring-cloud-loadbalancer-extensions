    ...
    public Object postProcessAfterInitialization(@NonNull Object bean, @NonNull String beanName) {
        if (bean instanceof Executor || bean instanceof TaskScheduler) {
            if (properties.getExecutor().accept(beanName)) {
                if (bean instanceof SchedulingTaskExecutor && bean instanceof TaskScheduler) {
                    log.info("Context propagation ENABLED for ~ThreadPoolTaskScheduler [{}]:[{}].", beanName, bean.getClass().getName());
                    return new ContextAwareThreadPoolTaskScheduler((SchedulingTaskExecutor) bean, (TaskScheduler) bean);
                    ...
                    if (bean instanceof ThreadPoolTaskExecutor) {
                        ...
                        return new ContextAwareThreadPoolTaskExecutor((SchedulingTaskExecutor) bean);
                    } else {
